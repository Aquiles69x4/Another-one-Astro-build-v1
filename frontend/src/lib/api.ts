...
const API_URL = (process.env.NEXT_PUBLIC_API_URL || 'https://backendotravez.vercel.app/api')
  .replace(/\/+$/, '');  // quita barras finales extra

class ApiClient {
  private async request(endpoint: string, options: RequestInit = {}) {
    // asegúrate de que endpoint comience con slash
    ...
    if (!response.ok) {
      ...
      throw new Error(error.message || (error as any).error || 'Request failed');
    }
